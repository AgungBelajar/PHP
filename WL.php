...
//prevent empty data
if (!isset($_POST['d']) || !isset($_POST['idwl'])) {
    $response = [
        'pesan' => 'Datetime or IDWL is empty!123 '. $_POST['idwl'] . $_POST['d'], 
    ];
} else {
    ...
    if ($idwl == 100 || $objDatetime < $oneYearAgo) {
        $datetime =  $date_now;
    } else {
    ...
    // $lvl_in = water_level($_POST["lvl_in"]);
    if ($idwl == 83) {
        $lvl_in = water_level($_POST["lvl_in"]) - 140;
    }else{
        $lvl_in = water_level($_POST["lvl_in"]);
    }
    ...
    $queryWl  = "SELECT * FROM water_level_list  JOIN water_level ON water_level.idwl=water_level_list.id WHERE water_level_list.id=$idwl";
    $resultQuery = mysqli_query($conn, $queryWl);
    $wldatalist = mysqli_fetch_array($resultQuery);

    $sql = "INSERT INTO water_level (datetime, lvl_in, lvl_out, lvl_act,  idwl)
        VALUES ('" . $datetime . "', '" . $lvl_in . "', '" . $lvl_out . "', '" . $lvl_act . "', '" . $idwl . "')";

    // $lvl_inBeforeEnd = '';
    $sqlDataBeforeEnd       = "SELECT lvl_in FROM water_level where idwl=$idwl  ORDER BY id DESC LIMIT 1";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $sqlCheckData = "SELECT * FROM water_level_idwm WHERE id = '" . $idwl . "'";
        $resultCheckData = mysqli_query($conn, $sqlCheckData);
        if ($resultCheckData) {
            if (mysqli_num_rows($resultCheckData) > 0) {
                conditionOfNotification($conn, $lvl_in, $result['lvl_in'], $idwl, $nameWl);
            }
        }
        // End Send Notifcation Firebase
    
        // Update Last Online IDWM
        $sqlLast = "SELECT * FROM water_level_idwm WHERE id = '" . $idwl . "' LIMIT 1";
        ...
        if ($resultLast) {
            if (mysqli_num_rows($resultLast) > 0) {
                ...
                if ($rawDateWm === null || $datePost > $dateWm) {
                    $queryUpdate = "UPDATE water_level_idwm SET level_in = '" . $lvl_in . "', level_out = '" . $lvl_out . "', last_online = '" . $datetime . "' WHERE id = '" . $idwl . "'";
        ...
        $queryUpdate = "UPDATE water_level_list SET last_online = '$date' WHERE id = '$idwl'";
        
        $result_update = mysqli_query($conn, $queryUpdate);
        
        // if ($result_update) {
        //     echo 'Sukses update water_lavel last_online dengan id: ' . $idwl;
        // } else {
...
function conditionOfNotification($connection, $lvlInPost, $lvlInLast, $idwl, $nameOfWl) {
    $topic = "News";
    $title = "SSMS Water Management";
    
    $dataOneHours = filterDataHours($connection, $idwl, 'PT1H');
...
function filterDataHours($connection, $idwl, $interval) {
    $filteredData = [];
    $lastDate = "";
    $dataQr = mysqli_query($connection, "SELECT * FROM water_level WHERE idwl = '$idwl' ORDER BY datetime DESC LIMIT 1");
    ...
    $filteredDataQr = mysqli_query($connection, "SELECT * FROM water_level WHERE datetime >= '$hoursAgoFormatted' AND idwl = '$idwl'");
